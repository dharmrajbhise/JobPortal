...
<!-- Navbar -->
  ...
  <form action="/user/courses/filter" method="post">
...
<c:forEach var="e" items="${course }">
<a href="${e.applyLink}" target="_blank">
 <div class="card animated fadeIn">
  <h5 class="card-header">${e.title}</h5>
  <div class="card-body">
  <h6 class="card-title">${e.company}</h6>
    <h6 class="card-title">${e.type}</h6>
    <p class="card-text">Description : ${e.description }</p>
    <p class="card-text">Mode : ${e.mode }</p>
    <p class="card-text">Availability : ${e.availability }</p>
    <a href="${e.applyLink}" class="btn btn-primary">Apply</a>
  </div>
</div>
</a>
</c:forEach> 
